    ...
    useEffect(() => {
        const loadExistingData = async () => {
            ...
            try {
                ...
                const hasBankTransactionFee = application.bankTransactionPaperFee && application.bankTransactionPaperFee.public_id;
                setIsReadOnly(hasBankTransactionFee && newStatus !== 'REJECTED');
                console.log('isReadOnly set to:', hasBankTransactionFee && newStatus !== 'REJECTED');

                if (hasBankTransactionFee) {
                    setFormData({
                        screenshotFile: null,
                        paymentId: application.bankTransactionPaperFee.paymentId || '',
                        screenshotUrl: application.bankTransactionPaperFee.url || '',
                    });
                    if (newStatus === 'APPROVED') {
                        const event = new CustomEvent('bankTransactionFeeSubmitted', { detail: { isValid: true, stepId: 6 } });
                        window.dispatchEvent(event);
                        console.log('Dispatched bankTransactionFeeSubmitted event:', { isValid: true, stepId: 6 });
        ...
        try {
            ...
            await uploadPaymentScreenshot(
                ...
                formData.screenshotFile,
                {
                    ...
                    amount: 1000, // Adjusted amount for bank transaction fee
                }
            );
            ...
            const event = new CustomEvent('bankTransactionFeeSubmitted', { detail: { isValid: false, stepId: 6 } });
            window.dispatchEvent(event);
            console.log('Dispatched bankTransactionFeeSubmitted event:', { isValid: false, stepId: 6 });
        } catch (error: any) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 py-6 sm:py-8 lg:py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl p-4 sm:p-6 lg:p-8 transform hover:scale-101 transition-all duration-300">
                <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4 sm:mb-6 text-center bg-gradient-to-r from-blue-600 to-purple-600 text-transparent bg-clip-text">
                    Bank Transaction Paper Fee Payment
                </h2>
                <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6 text-center leading-relaxed">
                    This fee covers the processing of your bank transaction paper for loan verification. It includes document validation and transaction confirmation.
                </p>
                ...
                <div className="bg-gradient-to-r from-gray-50 to-gray-100 p-4 sm:p-6 rounded-xl mb-4 sm:mb-6 border border-gray-200">
                    ...
                    <p className="text-xl sm:text-2xl font-bold text-blue-600 mb-2 sm:mb-4">Amount to Pay: ₹1000</p>
                </div>
