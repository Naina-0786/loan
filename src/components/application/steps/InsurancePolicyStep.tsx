...
export default function InsuranceFeeStep() {
    ...
    useEffect(() => {
        const loadExistingData = async () => {
            ...
            try {
                ...
                const hasInsuranceFee = application.insuranceFee && application.insuranceFee.public_id;
                setIsReadOnly(hasInsuranceFee && newStatus !== 'REJECTED');
                console.log('isReadOnly set to:', hasInsuranceFee && newStatus !== 'REJECTED');

                if (hasInsuranceFee) {
                    setFormData({
                        screenshotFile: null,
                        paymentId: application.insuranceFee.paymentId || '',
                        screenshotUrl: application.insuranceFee.url || '',
                    });
                    if (newStatus === 'APPROVED') {
                        const event = new CustomEvent('insuranceFeeSubmitted', { detail: { isValid: true, stepId: 7 } });
                        window.dispatchEvent(event);
                        console.log('Dispatched insuranceFeeSubmitted event:', { isValid: true, stepId: 7 });
        ...
        try {
            ...
            await uploadPaymentScreenshot(
                ...
                formData.screenshotFile,
                {
                    ...
                    amount: 1500, // Placeholder amount for insurance fee
                }
            );
            ...
            const event = new CustomEvent('insuranceFeeSubmitted', { detail: { isValid: false, stepId: 7 } });
            window.dispatchEvent(event);
            console.log('Dispatched insuranceFeeSubmitted event:', { isValid: false, stepId: 7 });
        } catch (error: any) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 py-6 sm:py-8 lg:py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl p-4 sm:p-6 lg:p-8 transform hover:scale-101 transition-all duration-300">
                ...
                <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6 text-center leading-relaxed">
                    This fee covers the insurance processing for your loan application. It includes policy validation and coverage confirmation.
                </p>
                ...
                <div className="bg-gradient-to-r from-gray-50 to-gray-100 p-4 sm:p-6 rounded-xl mb-4 sm:mb-6 border border-gray-200">
                    ...
                    <p className="text-xl sm:text-2xl font-bold text-blue-600 mb-2 sm:mb-4">Amount to Pay: ₹1500</p>
                </div>
